...
class ProfileList extends Component {
  ...
  profileDeleted = profile => {
    const newProfileList = this.state.profile.filter(profileFromState => profileFromState.getID() !== profile.getID());
    this.setState({
  ...
  filterFieldValueChange = event => {
    ...
    this.setState({
      filteredProfile: this.state.profile.filter(profile => {
        ...
        let AdressContainsValue = profile.getAdress().toLowerCase().includes(value);
        let SemesterContainsValue = profile.getSemester().toLowerCase().includes(value);
        let DegreeCourseContainsValue = profile.getDegreeCourse().toLowerCase().includes(value);
        let PreferencesContainsValue = profile.getPreferences().toLowerCase().includes(value);
        let PersonIdContainsValue = profile.getPersonId().toLowerCase().includes(value);

        return AgeContainsValue || AdressContainsValue || SemesterContainsValue || DegreeCourseContainsValue || PreferencesContainsValue || PersonIdContainsValue ;
      }),
      profileFilter: value
    });
  }
  ...
  render() {
    ...
    return (
      <div className={classes.root}>
        ...
        {
          profile.map(profile =>
            <ProfileListEntry key={profile.getID()} profile={profile}
              onProfileDeleted={this.profileDeleted}
            />)
        }
        <LoadingProgress show={loadingInProgress} />
        <ContextErrorMessage error={error} contextErrorMsg={`The list of profile could not be loaded.`} onReload={this.getProfile} />
